...
このSPAC法の基本的な考え方を理解するために、簡単な以下の問題設定を考えます。
...
ある観測点間の距離$r$がわかっていれば、$(左辺)$の値から波長$\lambda$が計算できる。そして、わかりにくいかもしれないが、式5は、ある特定の振動数 $\omega$ に対して計算されている。つまり、ここで計算される $\lambda, k$ は、特定の振動数 $\omega$ に対応する波長、波数である。つまりある振動数 $\omega$ と 波数 $k$ から計算される 位相速度 $c$は、次のように表される。
...
このようにすることで、特定の振動数 $\omega$に対する位相速度$c$を計算できる。

#### 補足

- F-K変換で得られるものと、この相関で得られる結果って何か違くない？ (F-k変換では振幅も求まるけど、相関では振幅は求まらないのでは？)
   - F-K変換は、時間領域の信号を振動数-波数領域に変換する手法であり、特定の振動数成分に対する振幅と位相を求めることができる。
   - 一方、空間自己相関法（SPAC法）は、観測点間の相関を利用して、波の位相速度を推定する手法である。
      - SPAC法では、暗黙の了解として、ある振動数 $\omega$ での波数 $k$ (あるいは 位相速度 $c$)は一つだけという暗黙の仮定があります。
...
F-K法（Frequency-Wavenumber Method）は、時空間領域の波動場データを振動数-波数領域に変換することで、波の伝播特性を解析する手法である。
...
F-K変換は、この時空間の周期性を同時に解析し、どの振動数とどの波数の組み合わせが卓越しているかを明らかにする。
...
これは、振動数$\omega$において、波数$(k_x, k_y)$を持つ波のエネルギーを表す。
...
3. **分散曲線の不連続**
   - 原因：モード判定の誤り、ノイズの影響
   - 対策：隣接振動数との連続性をチェック、品質の低いデータを除外
...
この関係により、異なる振動数の分散曲線から、異なる深さの地下構造情報を得ることができる。
